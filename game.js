...
const DEBUFF_CATALOG = {
  ...
  medium: [
    ...
    { id:'D-V2', label: '予測矢印 -2本',         key: 'predictArrowSub',       val: 2     },
    ...
    { id:'D-E2', label: 'スポーン -30%',         key: 'spawnIntervalMult',     val: -0.30 },
  ...
  heavy: [
    ...
    { id:'D-V2', label: '予測矢印 -3本(全消)',    key: 'predictArrowSub',       val: 3     },
    ...
    { id:'D-E2', label: 'スポーン -50%',         key: 'spawnIntervalMult',     val: -0.50 },
...
function applyKeyEffect(keyCol, target) {
  ...
  if (keyCol === 'blue') {
    ...
    const slowDur = Math.round(180 + mods.blueSlowFactor * 180 + mods.blueSlowDuration);
    for (const e of enemies) {
      if (!e.alive) continue;
    ...
    if (healAmt > 0) {
      const maxHp = CFG.PLAYER_HP_MAX + mods.maxHpBonus - mods.maxHpMalus;
...
function fullyUnlock(enemy) {
  ...
  if (mods.healOnKillChance > 0 && Math.random() < mods.healOnKillChance) {
    player.hp = Math.min(CFG.PLAYER_HP_MAX + mods.maxHpBonus - mods.maxHpMalus, player.hp + 1);
...
function update() {
  ...
  // Continuous enemy trickle (density increases with time + inscription mods)
  {
    const mods0    = getMods();
    const alive    = enemies.filter(e => e.alive).length;
    const baseMax  = CFG.ENEMY_COUNT + Math.floor(gameTime / 30) + Math.round(mods0.enemyCountBonus);
    const maxCount = Math.min(CFG.ENEMY_COUNT_MAX + Math.round(mods0.enemyCountBonus), baseMax);
    const baseInterval = Math.max(180, CFG.SPAWN_INTERVAL - Math.floor(gameTime / 20) * 40);
    const interval = Math.max(90, Math.round(baseInterval * (1 + mods0.spawnIntervalMult)));
    spawnTimer++;
