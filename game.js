...
function pointerUp(sx, sy) {
  ...
  if (state === State.IDLE) {
    if (dist < CFG.FLICK_MIN) {
      ...
      // Flick → move
      const spd = Math.min(dist / dt * 26, CFG.MAX_SPEED);
...
function update() {
  ...
  for (const e of enemies) {
    ...
    e.x += e.vx * sf;
    e.y += e.vy * sf;
    // Soft leash: steer back if strayed too far from player
    const d = Math.hypot(e.x - player.x, e.y - player.y);
    if (d > 1200) {
      const a = Math.atan2(player.y - e.y, player.x - e.x);
      e.vx += Math.cos(a) * 0.06;
      e.vy += Math.sin(a) * 0.06;
      const spd = Math.hypot(e.vx, e.vy);
      if (spd > CFG.ENEMY_BASE_SPEED * 1.5) {
        e.vx = (e.vx / spd) * CFG.ENEMY_BASE_SPEED * 1.5;
        e.vy = (e.vy / spd) * CFG.ENEMY_BASE_SPEED * 1.5;
      }
    }
...
function renderEnemy(e, t) {
  ...
  ctx.strokeStyle = focused ? '#2a2a2a' : '#888'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.arc(0, -5, 5.5, 0, Math.PI * 2); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(-4, -1.5); ctx.lineTo(-4, 7); ctx.lineTo(4, 7); ctx.lineTo(4, -1.5); ctx.stroke();
...
function renderPlayer(t) {
  ...
  ctx.strokeStyle = '#f0ede6'; ctx.fillStyle = '#f0ede6'; ctx.lineWidth = 1.5; ctx.lineCap = 'round';
  for (const ex of [-7, 7]) {
  ...
  ctx.beginPath(); ctx.arc(player.r * 0.68, -player.r * 0.68, 4.5, 0, Math.PI * 2);
  ctx.fill(); ctx.stroke();
