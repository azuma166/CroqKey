...
function resetGame() {
  ...
  infectedMode = false; infectionOrbSpawned = false;
...
function renderInscriptionEdgeIndicators() {
  ...
  for (const orb of inscriptionOrbs) {
    ...
    const col = isInfection ? '#cc44ff' : isEnemyBoost ? '#ff4400' : isFusion ? '#ffcc44' : RARITY_COLOR[orb.rarity];
    ...
    ctx.textAlign = 'center';
    ctx.fillText(isInfection ? '感染' : isEnemyBoost ? '敵増' : isFusion ? '融合' : '刻印', 0, 22);
