...
const BUFF_CATALOG = {
  light: [
    ...
    { id:'B-P1', label: 'ボム半径 +25%',       key: 'purpleBombRadMult',val: 0.25 },
...
function pointerUp(sx, sy) {
  ...
  if (state === State.IDLE) {
    if (dist < CFG.FLICK_MIN) {
      ...
      const spd = Math.min(dist / dt * 12 * Math.max(0.1, moveMult), CFG.MAX_SPEED);
...
function tryUnlock(dx, dy) {
  ...
  if (Math.abs(diff) < Math.max(5 * Math.PI / 180, tolerance)) {
    ...
    const critBonus = 2 + mods.critMult;
...
function applyKeyEffect(keyCol, target) {
  ...
  if (keyCol === 'blue') {
    ...
    const slowDur = Math.round(180 + mods.blueSlowFactor * 180 + (mods.blueSlowFactor < 0 ? 0 : 0));
    for (const e of enemies) {
      if (!e.alive) continue;
    ...
    if (healAmt > 0) {
      const maxHp = CFG.PLAYER_HP_MAX + mods.maxHpBonus;
