...
const BUFF_CATALOG = {
  light: [
    { id:'B-R1', label: '赤ダメ +15%',          key: 'redDmgMult',            val: 1.15 },
    { id:'B-R2', label: '赤クリット +0.3',       key: 'redCritMult',           val: 0.3  },
    ...
    { id:'B-D2', label: 'クリット倍率 +0.3',     key: 'critMult',              val: 0.3  },
    { id:'B-D3', label: '色不一致クリット 10%',  key: 'mismatchCritChance',    val: 0.10 },
    ...
    { id:'B-S3', label: 'コンボ17で無敵',        key: 'comboInvincible',       val: 17   },
    ...
    { id:'B-V3', label: '矢印表示 +20%',         key: 'ghostTimerMult',        val: 0.20 },
  ...
  medium: [
    ...
    { id:'B-R2', label: '赤クリット +0.6',        key: 'redCritMult',           val: 0.6  },
    ...
    { id:'B-D2', label: 'クリット倍率 +0.7',      key: 'critMult',              val: 0.7  },
    { id:'B-D3', label: '色不一致クリット 20%',   key: 'mismatchCritChance',    val: 0.20 },
    ...
    { id:'B-S3', label: 'コンボ11で無敵',         key: 'comboInvincible',       val: 11   },
    ...
    { id:'B-V3', label: '矢印表示 +40%',          key: 'ghostTimerMult',        val: 0.40 },
  ...
  heavy: [
    { id:'B-R1', label: '赤ダメ +50%',            key: 'redDmgMult',            val: 1.50 },
    { id:'B-R2', label: '赤クリット +1.0',        key: 'redCritMult',           val: 1.0  },
    ...
    { id:'B-D3', label: '色不一致クリット 35%',   key: 'mismatchCritChance',    val: 0.35 },
    ...
    { id:'B-S3', label: 'コンボ5で無敵',          key: 'comboInvincible',       val: 5    },
    ...
    { id:'B-V3', label: '矢印表示 +70%',          key: 'ghostTimerMult',        val: 0.70 },
    ...
    { id:'B-X5', label: '無傷ボーナス',           key: 'untouchedBonus',        val: 1    },
...
const DEBUFF_CATALOG = {
  light: [
    ...
    { id:'D-V3', label: '矢印表示 -20%',         key: 'ghostTimerMult',        val: -0.20 },
  ...
  medium: [
    ...
    { id:'D-V3', label: '矢印表示 -30%',         key: 'ghostTimerMult',        val: -0.30 },
    ...
    { id:'D-R5', label: '刻印頻度 -40%',         key: 'orbFreqMalus',          val: -0.40 },
  ...
  heavy: [
    ...
    { id:'D-V3', label: '矢印表示 -40%',         key: 'ghostTimerMult',        val: -0.40 },
    ...
    { id:'D-R3', label: 'コンボ減衰加速',        key: 'comboDrainAccel',       val: 1     },
    { id:'D-R4', label: '鍵ドロップ率 -65%',     key: 'keyDropChanceMult',     val: -0.65 },
    { id:'D-R5', label: '刻印頻度 -65%',         key: 'orbFreqMalus',          val: -0.65 },
...
function openDraft(orbIndex) {
  ...
  if (orb.type === 'enemy_boost') {
    ...
    orbAnnounce = { label: '幻影刻印', sub: '長押しで幻影モードに', color: '#88ffdd', age: 0, maxAge: 220, slowFrames: 90 };
...
let phantomEnemies    = [];   // enemies converted to orbs during phantom mode
...
function enterPhantomMode() {
  ...
  // Convert all alive enemies to phantom orbs
  for (const e of enemies) {
    if (!e.alive) continue;
    e.alive = false;
    phantomEnemies.push({ x: e.x, y: e.y, pulse: 0, alive: true });
  }
  orbAnnounce = { label: '幻影', color: '#88ffdd', age: 0, maxAge: 70, slowFrames: 35 };
}

function exitPhantomMode() {
  phantomMode = false;
  playPhantomExitSound();
  for (const pe of phantomEnemies) {
    if (pe.alive) {
      addFx('explosion', pe.x, pe.y, { color: '#88ffdd', maxAge: 45 });
      pe.alive = false;
    }
  }
  phantomEnemies.length = 0;
  screenFlash = { r: 200, g: 255, b: 240, alpha: 0.35 };
  orbAnnounce = { label: '幻影解除', color: '#aacccc', age: 0, maxAge: 75, slowFrames: 30 };
}
...
function resetGame() {
  ...
  phantomMode = false; phantomOrbSpawned = false; phantomOrbPickedUp = false; phantomEnemies.length = 0;
...
function update() {
  ...
  // Phantom enemy orb updates & collection
  if (phantomMode) {
    for (let i = phantomEnemies.length - 1; i >= 0; i--) {
      const pe = phantomEnemies[i];
      if (!pe.alive) { phantomEnemies.splice(i, 1); continue; }
      pe.pulse = (pe.pulse || 0) + 1;
      if (Math.hypot(player.x - pe.x, player.y - pe.y) < 32) {
        pe.alive = false;
        enemyBoostStacks++;
        playPhantomOrbCollectSound();
        addFx('explosion', pe.x, pe.y, { color: '#88ffdd', maxAge: 35 });
        phantomEnemies.splice(i, 1);
      }
    }
  }
...
function render() {
  ...
  // ── Phantom mode world filter ──
  if (phantomMode) {
    const phase = Math.floor(t * 5) % 3;
    if      (phase === 0) ctx.filter = 'invert(100%) hue-rotate(120deg) saturate(250%)';
    else if (phase === 1) ctx.filter = 'invert(85%) hue-rotate(240deg) saturate(350%) brightness(0.75)';
    else                  ctx.filter = 'invert(100%) hue-rotate(0deg) saturate(180%) brightness(1.2)';
  }
  ...
  if (zoomMod !== 0) {
    const scale = 1 + zoomMod * 0.4; // D-V1 +0.35 → 1.14 (zoom in); B-V1 -0.35 → 0.86 (zoom out)
  ...
  if (phantomMode) renderPhantomEnemies(t);
  if (state === State.CONNECTED && connectedEnemy) renderBeam(t);
  for (const e of enemies) if (e.alive) renderEnemy(e, t);
  renderPlayer(t);

  ctx.restore();
  ctx.filter = 'none'; // clear phantom filter before screen-space rendering

  // Phantom glitch scanlines overlay
  if (phantomMode) {
    ctx.save();
    const lineH = 4, skip = Math.floor(t * 30) % lineH;
    ctx.globalAlpha = 0.08;
    ctx.fillStyle = '#000000';
    for (let y = skip; y < h; y += lineH * 2) ctx.fillRect(0, y, w, lineH);
    // Occasional horizontal offset stripe
    const stripePhase = Math.floor(t * 8);
    if (stripePhase % 5 === 0) {
      const sy = (stripePhase * 73) % h;
...
function renderPhantomEnemies(t) {
  for (const pe of phantomEnemies) {
    if (!pe.alive) continue;
    const pulse = Math.sin(t * 4 + (pe.pulse || 0) * 0.1) * 0.5 + 0.5;
    const r = 14 + pulse * 4;
    ctx.save();
    ...
    // Spinning dashes
    ctx.setLineDash([4, 3]); ctx.lineDashOffset = -t * 50;
...
function renderOrbAnnounce(w, h) {
  ...
  // Sub-label (custom or default '取得')
  ctx.globalAlpha = alpha * 0.65;
  ctx.fillStyle = '#ffffff';
  ctx.font = '15px -apple-system, sans-serif';
  ctx.fillText(a.sub || '取得', cx, cy + 46);
