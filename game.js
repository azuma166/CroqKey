...
function renderPhantomEnemies(t) {
  // Minimal rendering: single circle + label per orb, no save/restore/lineDash
  ctx.font = 'bold 10px sans-serif';
  ctx.textAlign = 'center';
  for (const pe of phantomEnemies) {
    if (!pe.alive) continue;
    const pulse = Math.sin(t * 4 + (pe.pulse || 0) * 0.1) * 0.4 + 0.6;
    const r = 14;
    ctx.globalAlpha = pulse * 0.9;
    ctx.fillStyle = '#88ffdd';
    ctx.beginPath(); ctx.arc(pe.x, pe.y, r, 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha = 0.85;
    ctx.fillStyle = '#003322';
    ctx.fillText('敵増', pe.x, pe.y + 4);
  }
  ctx.globalAlpha = 1;
}
